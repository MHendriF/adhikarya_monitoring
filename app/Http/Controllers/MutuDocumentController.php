...
{
    protected $namadokumen = "MUTU";
    protected $submenu;
    protected $today;

    function __construct()
    {
        $this->submenu = 'Dokumen Mutu';
    ...
    {
        if(Auth::user()) {
            return view('pages.document.mutu.index');
        }
        else {
        ...
        if(Auth::user()) {
            ...
            return DataTables::eloquent($model)
                              ...
                              ->addColumn('action', function($dokumen){
                                    return
                                        '<a href="'. route('mutu.edit',$dokumen->id_dokumen) .'" data-toggle="tooltip" title="Edit" class="btn btn-success btn-sm btn-icon-anim btn-square mr-5" style="display: unset;"><i class="fa fa-pencil" style="font-size: 14px;"></i></a>'.
                                        '<a href="'. route('mutu.show',$dokumen->id_dokumen) .'" data-toggle="tooltip" title="Detail" class="btn btn-primary btn-sm btn-icon-anim btn-square mr-5" style="display: unset;"><i class="fa fa-eye" style="font-size: 14px;"></i></a>'.
                                        '<a href="'. route('mutu.delete',$dokumen->id_dokumen) .'" id="delete" data-toggle="tooltip" title="Delete" class="btn btn-danger btn-sm btn-icon-anim btn-square mr-5" style="display: unset;"><i class="fa fa-trash" style="font-size: 14px;"></i></a>';
                                })
                              ->make(true);
        }
    }
    ...
    public function create()
    {
        if(Auth::user()) {
            ...
            return view('pages.document.mutu.create', compact('listUser'));
        }
        else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
        ...
        if(Auth::user()) {
            ...
            if(!empty($files)):
                foreach ($files as $file):
                    ...
                    Storage::disk('dokumen_mutu')->put($fileName, file_get_contents($file));
                    ...
                    LampiranMutu::insert($data);
                endforeach;
            endif;

            Session::flash('create', 'New document was successfully added');
            return redirect()->route('mutu.index');
        }
        else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
    ...
    public function show($id)
    {
        if(Auth::user()){
          ...
          $attachments = $dokumen->lampiran_mutu;
          return view('pages.document.mutu.detail', compact('dokumen', 'picDokumen', 'attachments'));
        }else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
        }
    ...
    public function edit($id)
    {
        if(Auth::user()) {
            ...
            $listUser = User::pluck('nama_user', 'id_user')->all();
            $picDokumen = PicDokumen::where('id_dokumen', $dokumen->id_dokumen)->first();
            return view('pages.document.mutu.edit', compact('dokumen', 'listUser', 'picDokumen'));
        }
        else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
        ...
        if(Auth::user()) {
            try {
                if(Dokumen::where('id_dokumen', '=', $id)->delete())
                {
                    ...
                    $lampiran = LampiranMutu::where('id_dokumen', $id)->get();
                    foreach($lampiran as $fileLampiran) {
                        $filePath = 'dokumen\mutu\\'.$fileLampiran->nama_file;
    ...
    {
        $file = LampiranMutu::find($id);
        $filePath = 'dokumen/'.'mutu'.'/'.$file->nama_file;
