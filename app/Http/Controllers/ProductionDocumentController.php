...
class ProductionDocumentController extends Controller
{
    ...
    {
        if(Auth::user()) {
            return view('pages.document.production.index');
        }
        else {
    ...
    public function create()
    {
        if(Auth::user()) {
            ...
            return view('pages.document.production.create', compact('listUser'));
        }
        else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
    ...
    public function edit($id)
    {
        if(Auth::user()) {
            ...
            $listUser = User::pluck('nama_user', 'id_user')->all();
            $picDokumen = PicDokumen::where('id_dokumen', $dokumen->id_dokumen)->first();
            return view('pages.document.production.edit', compact('dokumen', 'listUser', 'picDokumen'));
        }
        else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
