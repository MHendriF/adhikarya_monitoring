...
use App\Models\LampiranFinance;
...
class FinanceDocumentController extends Controller
{
            ...
            return DataTables::eloquent($model)
                              ...
                              ->addColumn('action', function($dokumen){
                                    return
                                        ...
                                        '<a href="'. route('finance.show',$dokumen->id_dokumen) .'" data-toggle="tooltip" title="Detail" class="btn btn-primary btn-sm btn-icon-anim btn-square mr-5" style="display: unset;"><i class="fa fa-eye" style="font-size: 14px;"></i></a>'.
        ...
        if(Auth::user()) {
            ...
            if(!empty($files)):
                foreach ($files as $file):
                    ...
                    Storage::disk('dokumen_finance')->put($fileName, file_get_contents($file));

                    $data = array('id_dokumen' => $dokumen->id_dokumen,
                                'nama_file' => $fileName,
                                'path' => 'dokumen\finance',
                                'created_at' => $this->today,
                                'updated_at' => $this->today);
                    LampiranFinance::insert($data);
                endforeach;
            endif;
    ...
    public function show($id)
    {
        if(Auth::user()){
          ...
          $attachments = $dokumen->lampiran_finance;
          //return $attachments;
          return view('pages.document.finance.detail', compact('dokumen', 'picDokumen', 'attachments'));
        }else {
            Session::flash('error401', 'Full authentication is required to access this resource');
            return back();
        }
        ...
        if(Auth::user()) {
            try {
                if(Dokumen::where('id_dokumen', '=', $id)->delete())
                {
                    ...
                    $lampiran = LampiranFinance::where('id_dokumen', $id)->get();
    ...
    {
        $file = LampiranFinance::find($id);
        $filePath = 'dokumen/'.'finance'.'/'.$file->nama_file;
